...
function amountLabel(amount: string) {
  return amount === "0"
    ? "Any"
    : `${new Intl.NumberFormat(undefined, { maximumFractionDigits: 2 }).format(Number(formatUnits(BigInt(amount), 6)))} USDC`
}
              ...
              <tbody className="divide-y">
                {addresses.map((address) => (
                  <tr key={address.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">{amountLabel(address.amount)}</td>
