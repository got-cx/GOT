...
contract GOTSubscription is ReentrancyGuard {
    ...
    constructor(address gotFactory_, address spendPermissionManager_) {
        if (
            gotFactory_ == address(0) ||
            gotFactory_.code.length == 0 ||
            spendPermissionManager_ == address(0) ||
            spendPermissionManager_.code.length == 0
    ...
    function _validateBinding(
        ...
        if (
            ...
            binding.factory != address(GOT_FACTORY) ||
