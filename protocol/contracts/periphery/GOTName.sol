...
contract GOTName is EIP712, ReentrancyGuard, IGOTOwnerResolver {
  ...
  bytes32 public constant CLAIM_TYPEHASH =
    keccak256("Claim(bytes32 nameKey,address account,uint48 deadline)");
  ...
  event NameTransferred(
    bytes32 indexed nameKey, address indexed previousAccount, address indexed newAccount
  );
  ...
  function claim(Claim calldata claimData, bytes calldata verifierSignature) external nonReentrant {
    ...
    bytes32 digest = _hashTypedDataV4(
      keccak256(
        abi.encode(
          CLAIM_TYPEHASH, claimData.nameKey, claimData.account, claimData.deadline
        )
      )
    );
  ...
  function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
    return interfaceId == type(IGOTName).interfaceId
      || interfaceId == type(IGOTOwnerResolver).interfaceId || interfaceId == type(IERC165).interfaceId;
  }
}
