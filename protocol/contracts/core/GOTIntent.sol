...
contract GOTIntent is IGOTIntent {
  ...
  constructor(address treasury_, uint16 executionShareBps_, uint16 partnerShareBps_) {
    if (
      treasury_ == address(0) || executionShareBps_ == 0 || executionShareBps_ >= BPS_DENOMINATOR
        || partnerShareBps_ == 0 || partnerShareBps_ >= BPS_DENOMINATOR
    ...
    )
  {
    ...
    (processedAmount, ownerAmount, treasuryFee, partnerReward, executionReward) =
      _process(executor, effectiveOwner);
  }
  ...
  function executeFor(address executor)
    external
    onlyProxy
    returns (
  ...
  function _process(address executor, address effectiveOwner)
    internal
    returns (
      uint256 processedAmount,
      uint256 ownerAmount,
    ...
    )
  {
    ...
    (ownerAmount, treasuryFee, partnerReward, executionReward) =
      _allocation(previousTotal, newTotal, processedAmount, feeBps_, partner_ != address(0));
  ...
  function _resolveOwner() internal view returns (address effectiveOwner) {
    ...
    if (
      !supportsSuccess || supportsData.length != 32
        || abi.decode(supportsData, (uint256)) != 1
