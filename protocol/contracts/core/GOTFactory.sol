...
contract GOTFactory is IGOTFactory {
  ...
  constructor(
    ...
    if (
      implementation_ == address(0) || implementation_.code.length == 0 || treasury_ == address(0)
        || executionShareBps_ == 0 || executionShareBps_ >= BPS_DENOMINATOR
        || partnerShareBps_ == 0 || partnerShareBps_ >= BPS_DENOMINATOR || maxFeeBps_ == 0
        || maxFeeBps_ >= BPS_DENOMINATOR
    ) revert InvalidConfiguration();
    if (
      !_implementationMatches(
        implementation_, treasury_, executionShareBps_, partnerShareBps_
      )
    ) revert InvalidConfiguration();
  ...
  function previewAddress(IntentConfig calldata config) public view returns (address intentAddress) {
    ...
    intentAddress = GOTClones.predictDeterministicAddress(
      IMPLEMENTATION, _immutableArgs(config), salt, address(this)
    );
  ...
  function quoteGrossAmount(uint256 recipientAmount, uint16 feeBps_)
    public
    pure
    returns (uint256 gross)
  {
    ...
    gross = Math.mulDiv(
      recipientAmount - 1, BPS_DENOMINATOR, BPS_DENOMINATOR - feeBps_
    ) + 1;
    if (quoteOwnerAmount(gross, feeBps_) != recipientAmount) revert InvalidConfiguration();
  }

  function deployAndExecute(IntentConfig calldata config)
    external
    returns (
    ...
    )
  {
    ...
    if (intentAddress.code.length == 0) {
      bytes32 salt = _salt(config);
      address deployed = GOTClones.cloneDeterministic(
        IMPLEMENTATION, _immutableArgs(config), salt
      );
    ...
    (
      ...
      partnerReward,
      executionReward
    ) = IGOTIntent(intentAddress).executeFor(msg.sender);
  }
  ...
  {
    if (
      config.ownerSource == intentAddress || config.token == intentAddress
  ...
  function _implementationMatches(
    ...
    (bool treasuryOk, uint256 implementationTreasury) =
      _readUint(implementation_, bytes4(keccak256("TREASURY()")));
    (bool executionOk, uint256 implementationExecutionShare) =
      _readUint(implementation_, bytes4(keccak256("EXECUTION_SHARE_BPS()")));
    (bool partnerOk, uint256 implementationPartnerShare) =
      _readUint(implementation_, bytes4(keccak256("PARTNER_SHARE_BPS()")));
    (bool versionOk, uint256 implementationVersion) =
      _readUint(implementation_, bytes4(keccak256("PROTOCOL_VERSION()")));
    (bool lengthOk, uint256 implementationArgsLength) =
      _readUint(implementation_, bytes4(keccak256("IMMUTABLE_ARGS_LENGTH()")));
    return treasuryOk && executionOk && partnerOk && versionOk && lengthOk
