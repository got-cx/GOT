...
import {
  MockOwnerResolver,
  NotAResolver,
  MalformedOwnerResolver
} from "./mocks/MockOwnerResolver.sol";
...
contract GOTCoreTest is Test {
  ...
  function test_PositiveFeeWithoutPartnerLeavesPartnerShareAtTreasury() public {
    ...
    vm.prank(STRANGER);
    (,,, uint256 treasuryFee, uint256 partnerReward, uint256 executionReward) =
      factory.deployAndExecute(config);
  ...
  function _config(address ownerSource, bytes32 key, uint16 fee, address partner)
    internal
    view
    returns (IGOTFactory.IntentConfig memory config)
  {
