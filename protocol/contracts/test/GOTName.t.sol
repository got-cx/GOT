...
contract GOTNameTest is Test {
    ...
    function test_ERC1271VerifierAndDomainSeparation() public {
        ...
        GOTName.Claim memory claimData = GOTName.Claim(keccak256("x:@alice"), ALICE, uint48(block.timestamp + 1 hours));
    ...
    function test_ExpiredInvalidAndUnauthorizedClaimsFail() public {
        GOTName names = new GOTName(verifier);
        GOTName.Claim memory expired = GOTName.Claim(keccak256("expired"), ALICE, uint48(block.timestamp));
        ...
        GOTName.Claim memory wrongSigner = GOTName.Claim(keccak256("wrong"), ALICE, uint48(block.timestamp + 1 hours));
